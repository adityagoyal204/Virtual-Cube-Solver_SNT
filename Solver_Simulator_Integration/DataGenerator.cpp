...
cubestate id(cubestate state, int phase)
{
...
{   
    ...
    output_file<<"Data #"<<(i+1)<<"\n";
    output_file<<"Number of arguements in scramble: "<<argc;
    ...
    */
    ...
    while (++phase < 5) {
        ...
        cubestate currentId = id(currentState,phase), goalId = id(goalState,phase);
        if (currentId == goalId){

            output_file<<"\nNumber of moves in the phase: 0\nNone\nTime taken in microseconds: 0";
        ...
        while (1) {
            ...
            cubestate oldId = id(oldState,phase);
            ...
            for (int move = 0; move < 18; move++) {
                if (applicableMoves[phase] & (1 << move)) {
                    ...
                    cubestate newId = id(newState,phase);
                    ...
                    if (newDir && newDir != oldDir) {
                        ...
                        output_file<<"\nNumber of moves in the phase: "<<(int)algorithm.size()<<"\n";
                        ...
                        outmovescount+=(int)algorithm.size();

                        auto stop = high_resolution_clock::now();
                        auto duration = duration_cast<microseconds>(stop - start);
                        output_file << "\nTime taken in microseconds: "<< duration.count()  ;
    ...
    output_file<<"\n"<<"Total number of moves needed to solve: "<<outmovescount ;
    auto STOP= high_resolution_clock::now();
    auto DURATION = duration_cast<microseconds>(STOP - START);
    output_file << "\nTotal time taken in microseconds: "<< DURATION.count() << "\n" ;
    output_file<<"\n";
